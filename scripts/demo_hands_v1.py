...
from lib.hands.hands_trackerv1 import HandsTrackerv1
...
parser.add_argument("--save_path", default=r"./saves/v1_camera_test.mp4")
...
def main():
    ...
    writer = VideoWriter(
        opts.save_path
        if opts.camera
        else os.path.join(os.path.dirname(opts.save_path), "v1_" + os.path.basename(opts.inp)),
        cap,
        (w, h),
    )
