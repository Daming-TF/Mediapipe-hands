...
from lib.hands.hands_trackerv3 import HandsTrackerv3
...
parser.add_argument("--save_path", default=r"./saves/v3_camera_test.mp4")
...
def main():
    ...
    tracker = HandsTrackerv3(roi_mode=opts.roi_mode, debug=opts.debug)
    writer = VideoWriter(
        opts.save_path
        if opts.camera
        else os.path.join(os.path.dirname(opts.save_path), "v3_" + os.path.basename(opts.inp)),
        cap,
        (w, h),
    )
