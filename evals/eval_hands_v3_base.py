...
from lib.evals.hands_trackerv3 import HandsTrackerv3Base
...
parser.add_argument("--save_path", default=r"./saves/evalv3_base_camera_test.mp4")
...
def main():
    ...
    tracker = HandsTrackerv3Base(roi_mode=opts.roi_mode, debug=opts.debug)
    writer = VideoWriter(
        opts.save_path
        if opts.camera
        else os.path.join(os.path.dirname(opts.save_path), "evalv3-base_" + os.path.basename(opts.inp)),
        cap,
        (w, h),
    )
