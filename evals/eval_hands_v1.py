...
from lib.evals.hands_trackerv1 import HandsTrackerv1
...
parser.add_argument("--save_path", default=r"./saves/evalv1_camera_test.mp4")
...
def main():
    ...
    tracker = HandsTrackerv1(roi_mode=opts.roi_mode, debug=opts.debug)
    writer = VideoWriter(
        opts.save_path
        if opts.camera
        else os.path.join(os.path.dirname(opts.save_path), "evalv1_" + os.path.basename(opts.inp)),
        cap,
        (w, h),
    )
