...
class Handsv3Base(BasicHeatmapHands):
    ...
    def handle_hand_detector_bbox(self, img_bgr, left_hand, right_hand, boxes, scale=1.5):
        candi_box = list()
        for box in boxes:
            if (right_hand.landmark is not None) and (bb_iou(box, right_hand.landmark_to_box(box_factor=1.0)) > 0.25):
                continue
            elif (left_hand.landmark is not None) and (bb_iou(box, left_hand.landmark_to_box(box_factor=1.0)) > 0.25):
                continue
            else:
...
class HandsTrackerv3Base(object):
    def __init__(self, threshold=0.5, roi_mode=1, debug=False):
        self.drawer = Drawerv1(debug=debug, roi_mode=roi_mode)
        ...
        self.hand_model = Handsv3Base(self.roi_mode)
...
class Handsv3Align(BasicHeatmapHands):
    def __init__(self, roi_mode, img_size=160):
        super(Handsv3Align, self).__init__(img_size)

        # self.model_path = "./lib/models/mobilenetv2_160x160_alpha_140_coarserefine_align_data_v2_4_try02.onnx"
        self.model_path = "./lib/models/mobilenetv2_160x160_alpha_140_coarserefine_align_data_v2_4_test.onnx"
        self.roi_mode = roi_mode
        self.img_size = img_size
        self.num_joints = 21
        self.dim = 3
    ...
    def handle_hand_detector_bbox(self, img_bgr, left_hand, right_hand, boxes, scale=1.5):
        ...
        for box in boxes:
            ...
            if (right_hand.landmark is not None) and (bb_iou(box, right_hand.landmark_to_box(box_factor=1.0)) > 0.2):
                continue
            if (left_hand.landmark is not None) and (bb_iou(box, left_hand.landmark_to_box(box_factor=1.0)) > 0.2):
                continue
            else:
                candi_box.append(box)
...
class HandsTrackerv3BaseAlign(object):
    def __init__(self, img_size=160, threshold=0.5, roi_mode=1, debug=False):
        ...
        self.hand_model = Handsv3Align(self.roi_mode)
